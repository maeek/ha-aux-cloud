...
class AuxCloudFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            # Initialize the API without parameters and then login separately
            self._aux_cloud = AuxCloudAPI()

            # Call login with the parameters separately
            if await self.hass.async_add_executor_job(
                    lambda: self._aux_cloud.login(user_input[CONF_EMAIL], user_input[CONF_PASSWORD])
            ):
                config = {
